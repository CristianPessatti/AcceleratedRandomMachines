...
randomMachinesClassifier <- function(
  ...
  data <- localized_sampling(data, y_var = target, final_sample_size = nrow(data) * 0.5, heterogeneous_prop = 0.8, n_partitions = 10)
  ...
  attr(ensemble_model, "class") <- "randomMachinesClassifierLSClass"
  return(ensemble_model)
}

randomMachinesClassifierLSClass <- setClass("randomMachinesClassifierLSClass",
  slots = list(
    train = "data.frame",
    target = "character",
    kernels = "list",
...
predict.randomMachinesClassifierLSClass <- function(rmc_model, newdata, type = "class") {
